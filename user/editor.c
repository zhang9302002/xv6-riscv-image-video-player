...
int main(int argc, char *argv[])
{
	...
	if (argc == 1)
	{
		printf("please input the command as [editor file_name]\n");
	...
	while (1)
	{
		...
		{
			auto_show = 1;
			printf("enable show current contents after text changed.\n");
		}
		else if (strcmp(input, "hide") == 0)
		{
			auto_show = 0;
			printf("disable show current contents after text changed.\n");
		}
...
void com_save(char *text[], char *path)
{
	...
	if (fd == -1)
	{
		printf("save failed, file can't open:\n");
		//setProgramStatus(SHELL);
		exit(0);
	}
	if (text[0] == NULL)
